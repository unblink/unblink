...
import { agents, agentsLoading, fetchAgents } from '../../shared';
...
import { Agent } from '@/gen/unblink/agent/v1/agent_pb';

function AgentEditDialog(props: { agent: Agent; open: boolean; onOpenChange: (details: { open: boolean }) => void }) {
  const [name, setName] = createSignal(props.agent.name);
  const [instruction, setInstruction] = createSignal(props.agent.config?.instruction || '');
  const [selectedServiceIds, setSelectedServiceIds] = createSignal<string[]>(props.agent.serviceIds || []);

  onMount(() => {
    setName(props.agent.name);
    setInstruction(props.agent.config?.instruction || '');
    setSelectedServiceIds(props.agent.serviceIds || []);
  });
...
function AgentDeleteDialog(props: { agent: Agent; open: boolean; onOpenChange: (details: { open: boolean }) => void }) {
  const handleDelete = async () => {
    toaster.promise(async () => {
      await agentClient.deleteAgent({
        agentId: props.agent.id,
      });
      await fetchAgents();
    }, {
      ...
      <div onClick={() => setEditOpen(true)} class="bg-neu-850 border border-neu-750 rounded-lg p-4 hover:border-neu-700 transition-all cursor-pointer">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xs text-neu-500 line-clamp-1">
                Worker: <span class="text-neu-400">{props.agent.type}</span>
              </p>
            </div>
          </div>
          <Menu.Root>
        ...
        <div class="space-y-2">
          <div>
            ...
            <p class="mt-1 text-sm text-neu-300 line-clamp-3">{props.agent.config?.instruction}</p>
          </div>
        </div>
        <div class="mt-3 pt-3 border-t border-neu-750 flex items-center justify-between text-xs text-neu-500">
          <span>{props.agent.serviceIds?.length || 0} service{props.agent.serviceIds?.length !== 1 ? 's' : ''}</span>
        </div>
      </div>
