...
func NewReconnector(config *Config) *Reconnector {
	maxAttempts := config.Reconnect().MaxNumAttempts
	if maxAttempts <= 0 {
...
// Run runs the node client with automatic reconnection
func (r *Reconnector) Run(createClient func() *NodeClient) {
	attempt := int64(0)
