...
func NewWebRTCSessionManager(relay *Relay) *WebRTCSessionManager {
...
func (m *WebRTCSessionManager) NewSession(offerSDP string, service *Service) (string, string, error) {
		log.Printf("[WebRTC] Creating new session for service %s (type=%s)", service.ID, service.Type())

	// Get node connection
	nodeConn, ok := m.relay.GetNodeConnection(service.NodeID)
	if !ok {
		log.Printf("[WebRTC] Node %s not connected", service.NodeID)
		return "", "", fmt.Errorf("node %s not connected", service.NodeID)
	}

	log.Printf("[WebRTC] Got node connection, opening bridge...")

	// Open bridge through node to the service
	ctx := context.Background()
	bridgeID, _, err := nodeConn.OpenBridge(ctx, service)
	if err != nil {
		log.Printf("[WebRTC] Failed to open bridge: %v", err)
		return "", "", fmt.Errorf("open bridge: %w", err)
	}

	log.Printf("[WebRTC] Opened bridge %s to service %s", bridgeID, service.ID)

	// Get bridge data channel
	dataChan, ok := nodeConn.GetBridgeDataChan(bridgeID)
	if !ok {
		log.Printf("[WebRTC] Failed to get bridge data channel")
		nodeConn.CloseBridge(ctx, bridgeID)
		return "", "", fmt.Errorf("get bridge data channel")
	}

	// Create bridge connection wrapper
	bridgeConn := NewBridgeConn(bridgeID, nodeConn, dataChan)

	log.Printf("[WebRTC] Creating media source...")
	// Create source (RTSP or MJPEG based on service type)
	source, err := NewMediaSource(service, bridgeID, bridgeConn)
	if err != nil {
		log.Printf("[WebRTC] Failed to create media source: %v", err)
		bridgeConn.Close()
		nodeConn.CloseBridge(ctx, bridgeID)
	...
	for _, m := range medias {
		for _, c := range m.Codecs {
			log.Printf("[WebRTC] Source media: %s/%s", m.Kind, c.Name)
		}
	}
	for _, m := range webrtcMedias {
		for _, c := range m.Codecs {
			log.Printf("[WebRTC] Browser offer: %s (%s), codec: %s", m.Kind, m.Direction, c.Name)
		}
	}
	...
	answerSDP, err := wconn.GetCompleteAnswer()
	if err != nil {
		pc.Close()
		source.Close()
		bridgeConn.Close()
